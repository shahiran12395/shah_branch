...
// Payment Informations  
  ...
  Route::get('/general-settings/billplz/{status}', 'Admin\GeneralSettingController@billplz')->name('admin-gs-billplz');
...
Route::prefix('supplier')->group(function () {
    ...
    Route::group(['middleware'=>'supplier'],function(){
        // Vendor Dashboard
        Route::get('/dashboard', 'Supplier\SupplierController@index')->name('supplier-dashboard');

        //------------ SUPPLIER NOTIFICATION SECTION ------------

        // Order Notification
        Route::get('/order/notf/show/{id}', 'Supplier\NotificationController@order_notf_show')->name('supplier-order-notf-show');
        Route::get('/order/notf/count/{id}','Supplier\NotificationController@order_notf_count')->name('supplier-order-notf-count');
        Route::get('/order/notf/clear/{id}','Supplier\NotificationController@order_notf_clear')->name('supplier-order-notf-clear');
        // Order Notification Ends

        // Product Notification Ends

        //------------ SUPPLIER NOTIFICATION SECTION ENDS ------------

        //------------ VENDOR PRODUCT SECTION ------------

        Route::get('/products/datatables', 'Supplier\ProductController@datatables')->name('supplier-prod-datatables'); //JSON REQUEST
        Route::get('/products', 'Supplier\ProductController@index')->name('supplier-prod-index');

        Route::post('/products/upload/update/{id}', 'Supplier\ProductController@uploadUpdate')->name('supplier-prod-upload-update');

        // CREATE SECTION
        Route::get('/products/types', 'Supplier\ProductController@types')->name('supplier-prod-types');
        Route::get('/products/physical/create', 'Supplier\ProductController@createPhysical')->name('supplier-prod-physical-create');
        Route::get('/products/digital/create', 'Supplier\ProductController@createDigital')->name('supplier-prod-digital-create');
        Route::get('/products/license/create', 'Supplier\ProductController@createLicense')->name('supplier-prod-license-create');
        Route::post('/products/store', 'Supplier\ProductController@store')->name('supplier-prod-store');
        Route::get('/products/import', 'Supplier\ProductController@import')->name('supplier-prod-import');
        Route::post('/products/import-submit', 'Supplier\ProductController@importSubmit')->name('supplier-prod-importsubmit');
        // CREATE SECTION

        // EDIT SECTION
        Route::get('/products/edit/{id}', 'Supplier\ProductController@edit')->name('supplier-prod-edit');
        Route::post('/products/edit/{id}', 'Supplier\ProductController@update')->name('supplier-prod-update');
        // EDIT SECTION ENDS

        // STATUS SECTION
        Route::get('/products/status/{id1}/{id2}', 'Supplier\ProductController@status')->name('supplier-prod-status');
        // STATUS SECTION ENDS

        // DELETE SECTION
        Route::get('/products/delete/{id}', 'Supplier\ProductController@destroy')->name('supplier-prod-delete');
        // DELETE SECTION ENDS

        //------------ SUPPLIER PRODUCT SECTION ENDS------------

        //------------ SUPPLIER SUBCATEGORY SECTION ------------

        Route::get('/load/subcategories/{id}/', 'Supplier\SupplierController@subcatload')->name('supplier-subcat-load'); //JSON REQUEST

        //------------ SUPPLIER SUBCATEGORY SECTION ENDS------------
    });
});
...
// ************************************ FRONT SECTION **********************************************
  ...
  Route::get('/billplz/callback', 'Front\BillplzController@callback')->name('billplz.callback');
  Route::get('/billplz/redirect', 'Front\BillplzController@redirect')->name('billplz.redirect');
  ...
  Route::post('/billplz', 'Front\BillplzController@billplz')->name('billplz.submit');
