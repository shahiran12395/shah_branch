...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('status', function (Product $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('supplier-prod-status', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . $this->vendor_language->lang713 . '</option><<option data-val="0" value="' . route('supplier-prod-status', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . $this->vendor_language->lang714 . '</option>/select></div>';
            })
            ->addColumn('action', function (Product $data) {
                return '<div class="action-list"><a href="' . route('supplier-prod-edit', $data->id) . '"> <i class="fas fa-edit"></i>' . $this->vendor_language->lang715 . '</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-eye"></i> ' . $this->supplier_language->lang716 . '</a><a href="javascript:;" data-href="' . route('supplier-prod-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
    ...
    {
        return view('supplier.product.types');
    }
    ...
    public function createPhysical()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default', '=', 1)->first();
        return view('supplier.product.create.physical', compact('cats', 'sign'));
    }
    ...
    public function createDigital()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default', '=', 1)->first();
        return view('supplier.product.create.digital', compact('cats', 'sign'));
    }
    ...
    public function createLicense()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default', '=', 1)->first();
        return view('supplier.product.create.license', compact('cats', 'sign'));
    }
    ...
    public function import()
    {
        ...
        $cats = Category::all();
        $sign = Currency::where('is_default', '=', 1)->first();
        return view('supplier.product.productcsv', compact('cats', 'sign'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        if ($prods < $package->allowed_products) {
            ...
            //--- Redirect Section
            $msg = 'New Product Added Successfully.<a href="' . route('supplier-prod-index') . '">View Product Lists.</a>';
            return response()->json($msg);
            //--- Redirect Section Ends
        } else {
            //--- Redirect Section
    ...
    public function edit($id)
    {
        ...
        if ($data->type == 'Digital')
            return view('supplier.product.edit.digital', compact('cats', 'data', 'sign'));
        elseif ($data->type == 'License')
            return view('supplier.product.edit.license', compact('cats', 'data', 'sign'));
        else
            return view('supplier.product.edit.physical', compact('cats', 'data', 'sign'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        //--- Redirect Section        
        $msg = 'Product Updated Successfully.<a href="' . route('supplier-prod-index') . '">View Product Lists.</a>';
        return response()->json($msg);
        //--- Redirect Section Ends    
    }
